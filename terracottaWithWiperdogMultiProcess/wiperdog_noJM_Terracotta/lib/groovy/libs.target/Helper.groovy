...
class Helper {
	...
	def Helper(groovyShell, scheduler, properties1){
		sched = scheduler
		shell = groovyShell
	...
	def processJob(file){
		...
		def tmp = [:]
		tmp[ResourceConstants.JOB_DIRECTORY] = properties.get(ResourceConstants.JOB_DIRECTORY)
		JobDataMap dataMap = new JobDataMap(tmp)
		def jobClass = shell.getClassLoader().loadClass("CustomJob")
		JobDetail job = JobBuilder.newJob(jobClass).withIdentity(jobProp["JOB"].name).usingJobData(dataMap).storeDurably(true).build()
